...
- **Step 4** :
    - Run **binarization steps(for faster I/O)** as follow:
        ```sh
        export PYTHONPATH=.
        CUDA_VISIBLE_DEVICES=0 python data_gen/tts/vocoder_binarizer.py --config ./modules/FastDiff/config/FastDiff.yaml
        ```
...
- **Step 1** :
    - Train the prodiff teacher in the cmd: 
        ```sh
         CUDA_VISIBLE_DEVICES=0 python tasks/run.py --config ./modules/FastDiff/config/FastDiff.yaml  --exp_name kallu_mama --reset
        ```
